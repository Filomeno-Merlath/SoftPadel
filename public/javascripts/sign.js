...
async function newUser(){
    ...
    var url = "https://api.mapbox.com/geocoding/v5/mapbox.places/"+address+" "+city+".json?limit=1&access_token=pk.eyJ1IjoiZmlsb21lbm9tIiwiYSI6ImNrdjEyaGg1NDBjdXkydW92dHVib2RtbXUifQ.YgXHEY0WXf8ptKQJ1wkUyQ";
    console.log(url);
...
async function newField(){
    let address = document.querySelector("#address").ariaValueMax;
    let sel = document.querySelector("#cities");
    let city = sel.options[sel.selectedIndex].text;
    var url = "https://api.mapbox.com/geocoding/v5/mapbox.places/"+address+" "+city+".json?limit=1&access_token=pk.eyJ1IjoiZmlsb21lbm9tIiwiYSI6ImNrdjEyaGg1NDBjdXkydW92dHVib2RtbXUifQ.YgXHEY0WXf8ptKQJ1wkUyQ";
    console.log(url);
 
     fetch(url).then(response => response.json()).then(async element =>{
         console.log(element);
         try{
             let data;
             data = {
